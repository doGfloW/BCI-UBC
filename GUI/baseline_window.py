# Baseline oddball task

'''
This task displays either a blue or green circle and records when user hits space
It pumps data about what happens when to an LSL stream
It also receives EEG data from a muse, or simulates it
This data is recorded along with events
...
The data is stored with times leftized (timestamp 0 when stim first displayed, for each trial)
...
class baseline_win(QWidget):
    def __init__(self, hardware = None, model = None, sim_type = None, 
        ...
        # check if the task is performed live or simulated
        # check hardware type and assign board_id
        ...
        # variable to actually display a stimulus of specified color
        ...
        # trials is a list of random or oddball in the order that we will use them (randomized, 50% right)
        self.total_trials = 10
        right_trials = self.total_trials // 2
        left_trials = self.total_trials - right_trials
        self.trials = [self.stim_type['left']]*left_trials + [self.stim_type['right']]*right_trials
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Qt.Key_Space:
            ...
            print('hardware {} running trial {}'.format(self.hardware_connected, self.running_trial))
